...
uint32 i2c_init(I2Cn_e i2cn, uint32 baud)
{
    if(i2cn == I2C0)
    {
        ...
        {
            port_init (I2C0_SCL, ALT2 | ODO | PULLUP );
        }
        else
            ASSERT(0);                              //上诉条件都不满足，直接断言失败了，设置管脚有误？
        if((I2C0_SDA == PTB1) || (I2C0_SDA == PTB3) || (I2C0_SDA == PTD9) )
            port_init (I2C0_SDA, ALT2 | ODO | PULLUP );
        else
            ASSERT(0);                              //上诉条件都不满足，直接断言失败了，设置管脚有误？
    }
    else
    {
        ...
        /* 配置 I2C1功能的 GPIO 接口 */
        if(I2C1_SCL == PTE1)
            port_init (I2C1_SCL, ALT6 | ODO | PULLUP );
        else if(I2C1_SCL == PTC10)
            port_init (I2C1_SCL, ALT2 | ODO | PULLUP );
        else
            ASSERT(0);                          //上诉条件都不满足，直接断言失败了，设置管脚有误？
        if(I2C1_SDA == PTE0)
            port_init (I2C1_SDA, ALT6 | ODO | PULLUP );
        else if (I2C1_SDA == PTC11)
            port_init (I2C1_SDA, ALT2 | ODO | PULLUP );
        else
            ASSERT(0);                          //上诉条件都不满足，直接断言失败了，设置管脚有误？
    }
    /* 设置频率 */
